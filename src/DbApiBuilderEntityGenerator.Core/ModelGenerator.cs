...
using Model = DbApiBuilderEntityGenerator.Core.Metadata.Generation.Model;
using Property = DbApiBuilderEntityGenerator.Core.Metadata.Generation.Property;
using PropertyCollection = DbApiBuilderEntityGenerator.Core.Metadata.Generation.PropertyCollection;
...
public partial class ModelGenerator
{
  ...
  public EntityContext Generate(GeneratorOptions options, DatabaseModel databaseModel, IRelationalTypeMappingSource typeMappingSource)
  {
    ...
    {
      if (IsIgnored(table, _options.Exclude.Tables))
      {
  ...
  private Entity CreateEntity(EntityContext entityContext, DatabaseTable tableSchema)
  {
    ...
    var entityClass = ToClassName(tableSchema.Name, tableSchema.Schema);
  ...
  private void CreateProperties(Entity entity, DatabaseTable tableSchema)
  {
    ...
    foreach (var column in columns)
    {
      ...
      if (IsIgnored(column, _options.Exclude.Columns))
      {
      ...
      if (property == null)
      {
        property = new Metadata.Generation.Property()
        {
          Entity = entity,
      ...
      foreach (var selection in _options.Renaming.Properties.Where(p => p.Expression.HasValue()))
      {
      ...
      if (property.IsRowVersion == true && property.SystemType == typeof(byte[]))
      {
        ...
        //   _ => typeof(byte[])
        // };
        property.SystemType = typeof(long);
      }
  ...
  private void CreateRelationships(EntityContext entityContext, Entity entity, DatabaseTable tableSchema)
  {
    foreach (var foreignKey in tableSchema.ForeignKeys)
    {
      ...
      if (IsIgnored(foreignKey.PrincipalTable, _options.Exclude.Tables))
      {
  ...
  {
    var naming = _options.RelationshipNaming;
  ...
  {
    var tableNaming = _options.EntityNaming;
    var entityNaming = _options.EntityNaming;

    if (tableNaming != EntityNaming.Plural && entityNaming == EntityNaming.Plural)
      name = name.Pluralize(false);
    else if (tableNaming != EntityNaming.Singular && entityNaming == EntityNaming.Singular)
      name = name.Singularize(false);

    var rename = name;
    foreach (var selection in _options.Renaming.Entities.Where(p => p.Expression.HasValue()))
    {
      if (selection.Expression.IsNullOrEmpty())
        continue;
